...
export default {
  ...
  actions: {
    ...
    async AddCashExpense({ commit }, payload) {
      commit('setLoading', true);
      const filters = new FormData();
      filters.append('expense_date', payload.expense_date);
      filters.append('expense_head', payload.expense_head);
      filters.append('ledger_id', payload.ledger_id);
      filters.append('amount', payload.amount);
      filters.append('reference', payload.reference);
      filters.append('remarks', payload.remarks);
      filters.append('added_by', payload.added_by);
      filters.append('create_new_expense', 'new');
      const response = await API.smart(PATH, filters);
      commit('setLoading', false);
      return response;
    },
